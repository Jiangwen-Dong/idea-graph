...
CATEGORY_WEIGHTS = {
    "expert_style_quality": 0.50,
    "benchmark_alignment": 0.50,
}
...
def _latest_snapshot_values(graph: IdeaGraph) -> tuple[float, float, float, float]:
    ...
    utility_norm = _clamp(snapshot.utility / 10.0)
    return (
...
def _score_novelty(sections: dict[str, str], metadata: dict[str, Any]) -> IdeaMetricScore:
    proposal_core = " ".join(
        item
        for item in (
            sections["title"],
    ...
    method_gap = 1.0 - _token_f1(
        " ".join([sections["hypothesis"], sections["method"]]),
        sections["existing_methods"],
    )
    proposal_specificity = _mean(
        [
            _section_length_quality(sections["hypothesis"]),
            _section_length_quality(sections["method"]),
        ]
    )
    normalized = _clamp(
        (0.50 * (1.0 - max_reference_similarity))
        + (0.30 * method_gap)
        + (0.20 * proposal_specificity)
    )
    rationale = (
        "Approximates novelty from the final proposal alone by rewarding distance from nearby references, "
        "a clear gap from the described existing methods, and a concrete hypothesis-method pair."
    )
    return _available_metric(
        key="novelty",
        ...
        rationale=rationale,
        signals={
            "reference_gap": 1.0 - max_reference_similarity,
            "method_gap": method_gap,
            "proposal_specificity": proposal_specificity,
        },
    )
...
def _score_significance(sections: dict[str, str], topic: str) -> IdeaMetricScore:
    ...
    topic_alignment = _token_f1(
        " ".join([sections["problem"], sections["significance"]]),
        _clean_topic_prompt(topic),
    )
    normalized = _clamp(
...
def _score_feasibility(sections: dict[str, str], dataset_items: list[str], metric_items: list[str]) -> IdeaMetricScore:
    experiment_specificity, specificity_signals = _score_experiment_specificity(sections, dataset_items, metric_items)
    method_quality = _section_length_quality(sections["method"])
    caveat_quality = _section_length_quality(sections["caveats"])
    method_eval_alignment = _token_f1(
        " ".join([sections["hypothesis"], sections["method"]]),
        sections["evaluation"],
    )
    normalized = _clamp(
        (0.35 * experiment_specificity)
        + (0.25 * method_quality)
        + (0.20 * caveat_quality)
        + (0.20 * method_eval_alignment)
    )
    rationale = (
        "Approximates feasibility from the written proposal by combining method concreteness, caveat awareness, "
        "evaluation specificity, and whether the evaluation text actually tests the proposed mechanism."
    )
    return _available_metric(
        key="feasibility",
        ...
        signals={
            ...
            "method_eval_alignment": method_eval_alignment,
            **specificity_signals,
        },
    )
...
def _score_effectiveness(
    ...
    proposal_chain_signal = _mean(
        [
            1.0 if sections["problem"] else 0.0,
    ...
    topic_alignment = _token_f1(
        " ".join([sections["problem"], sections["hypothesis"], sections["method"]]),
        topic,
    )
    method_eval_alignment = _clamp(
        _token_f1(" ".join([sections["hypothesis"], sections["method"]]), sections["evaluation"])
    )
    experiment_specificity, specificity_signals = _score_experiment_specificity(sections, dataset_items, metric_items)
    normalized = _clamp(
        (0.25 * proposal_chain_signal)
        + (0.25 * topic_alignment)
        + (0.20 * method_eval_alignment)
        + (0.30 * experiment_specificity)
    )
    rationale = (
        "Approximates effectiveness and testability from the final proposal by checking whether it forms a coherent "
        "problem-to-method-to-evaluation chain that stays aligned with the topic and includes concrete tests."
    )
    return _available_metric(
        key="effectiveness",
...
def _score_grounding(sections: dict[str, str], metadata: dict[str, Any], grounding: dict[str, Any]) -> IdeaMetricScore:
    ...
    experiment_specificity, specificity_signals = _score_experiment_specificity(sections, dataset_items, metric_items)
    reference_texts = _reference_texts(metadata)
    grounding_focus = " ".join(
        item for item in (sections["existing_methods"], sections["motivation"], sections["evaluation"]) if item
    )
    reference_alignment = max((_token_f1(grounding_focus, reference) for reference in reference_texts), default=0.0)
    normalized = _clamp(
        (0.40 * existing_methods_alignment)
        + (0.35 * experiment_specificity)
        + (0.25 * reference_alignment)
    )
    rationale = (
        "Measures literature grounding from the final proposal by checking whether the idea situates itself relative "
        "to the provided references and uses benchmark-relevant experimental context."
    )
    return _available_metric(
        key="grounding",
...
def _topic_keywords(metadata: dict[str, Any], topic: str) -> list[str]:
    ...
    keywords: list[str] = []
    direct_keyword = _clean_text(metadata.get("keyword"))
    if direct_keyword:
        keywords.append(direct_keyword)
    ...
    if not keywords:
        keywords.append(_clean_topic_prompt(topic))
    return keywords
...
def _score_topic_alignment(sections: dict[str, str], metadata: dict[str, Any], topic: str) -> IdeaMetricScore:
    ...
    cleaned_topic = _clean_topic_prompt(topic)
    similarity = _token_f1(proposal_focus, cleaned_topic)
    keyword_signal = _keyword_score(proposal_focus, _topic_keywords(metadata, cleaned_topic))
...
def evaluate_graph(graph: IdeaGraph) -> IdeaEvaluation:
    ...
    metrics = [
        _score_novelty(sections, metadata),
        _score_significance(sections, graph.topic),
        _score_feasibility(sections, dataset_items, metric_items),
        _score_effectiveness(sections, dataset_items, metric_items, graph.topic),
        _score_clarity_and_coherence(sections),
        _score_grounding(sections, metadata, grounding),
    ...
    notes = [
        "Expert-style quality metrics are deterministic output-only surrogates inspired by recent idea-evaluation rubrics, especially novelty, feasibility, effectiveness, clarity, and scientific promise.",
        "Benchmark-alignment metrics are deterministic output-only surrogates inspired by AI Idea Bench 2025 and related benchmark settings, including topic faithfulness, literature grounding, and concordance with held-out target content when available.",
        "Graph-process diagnostics are reported separately from the main outcome score so that non-graph baselines are not structurally penalized.",
        "These scores remain local deterministic surrogates rather than benchmark-native or human-review judgments, so they are most reliable for controlled iteration and ablation comparison.",
    ]
